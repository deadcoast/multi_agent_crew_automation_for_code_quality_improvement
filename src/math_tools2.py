...
try:
    code_representation_collection = chroma_client.get_or_create_collection(
        name="code_representations", metadata={"hnsw:space": "cosine"}
    )
...
class CodeMathRepresentationTool(BaseTool):
    ...
    def _extract_basic_metrics(self, tree: ast.AST) -> Dict[str, float]:
        ...
        for node_type in ast.AST.__subclasses__():
            ...
            node_counts[type_name] = len(
                [n for n in ast.walk(tree) if isinstance(n, node_type)]
            )
        ...
        operands = len(
            [n for n in ast.walk(tree) if isinstance(n, (ast.Name, ast.Num, ast.Str))]
        )
        ...
        unique_operands = len(
            {
                getattr(n, "id", str(n))
                for n in ast.walk(tree)
                if isinstance(n, (ast.Name, ast.Num, ast.Str))
            }
        )
            ...
            if isinstance(node, ast.Assign):
                for target in node.targets:
                    if isinstance(target, ast.Name):
                        ...
                        for name_node in [
                            n for n in ast.walk(node.value) if isinstance(n, ast.Name)
                        ]:
                            if name_node.id in var_assignments:
                                used_var_id = f"Var_{name_node.id}"
                                G.add_node(
                                    used_var_id, type="Variable", name=name_node.id
                                )
    ...
    def _analyze_graph(
        self, G: nx.DiGraph, prefix: str, calculate_spectra: bool
    ) -> Dict[str, float]:
        ...
        if G.number_of_nodes() > 1:
            try:
                metrics[f"{prefix}_avg_shortest_path"] = int(
        ...
        if G.number_of_nodes() > 1:
            try:
                ...
                metrics[f"{prefix}_max_centrality"] = int(
                    max(centrality.values()) if centrality else 0
                )
                metrics[f"{prefix}_avg_centrality"] = int(
        ...
        if calculate_spectra and G.number_of_nodes() > 1:
            try:
                ...
                eigenvalues = sorted(
                    nx.adjacency_matrix(G).todense().real, reverse=True
                )[:3]
                for i, val in enumerate(eigenvalues):
                    metrics[f"{prefix}_eigenvalue_{i}"] = int(float(val))
            except Exception:
                self._graph_values(metrics, prefix)
        else:
            self._graph_values(metrics, prefix)

        return {k: float(v) for k, v in metrics.items()}

    def _graph_values(self, metrics, prefix):
    ...
    def _estimate_algorithmic_complexity(self, tree: ast.AST) -> Dict[str, float]:
        ...
        class LoopVisitor(ast.NodeVisitor):
            def visit_For(self, node):
                self._extracted_from_visit_For_11(node)

            def _extracted_from_visit_For_11(self, node):
        ...
        max_depth = max(loop_depths, default=0)
...
class CodeVectorizationTool(BaseTool):
    ...
    def _run(
        self,
        code_id: str,
        math_representation: Dict[str, Any],
        ...
        try:
            ...
            filtered_metrics = {
                k: v
                for k, v in math_representation.items()
            ...
            if store_in_db:
                try:
                    metadata = {"keys": ",".join(sorted_keys), "source": code_id}
                    # Convert vector to list of floats explicitly for compatibility with vector DB
                    float_vector = [float(v) for v in normalized_vector]
            ...
            # Ensure return type is List[float]
            return [float(v) for v in normalized_vector]
...
class AlgorithmQualityAssessmentTool(BaseTool):
    ...
    def _run(
        ...
        try:
            return self._vector_validator(
                vector, reference_patterns, code_id, visualize
            )
        except Exception as e:
            self._traceback(
                'Error in algorithm quality assessment: ', e
            )
            return {"error": str(e)}

    def _vector_validator(self, vector, reference_patterns, code_id, visualize):
        ...
        n_components = min(
            3, all_vectors_array.shape[1], all_vectors_array.shape[0]
        )
        ...
        coords_3d = {
            name: vectors_3d[i].tolist() for i, name in enumerate(pattern_names)
        }
        ...
        quality_score = 0.0
        if (
            "optimal_time" in distances
            and "optimal_space" in distances
            and "poor_quality" in distances
        ):
            # Higher score means better quality (0-100 scale)
            optimal_dist = min(
                distances["optimal_time"], distances["optimal_space"]
            )
    ...
    def _generate_3d_visualization(
        self, coords_3d: Dict[str, List[float]], highlight_id: str
    ) -> None:
        """Generate 3D visualization of algorithm vectors."""
        try:
            self._create_3d_axes(highlight_id, coords_3d)
        except Exception as e:
            self._traceback(
                'Error generating 3D visualization: ', e
            )

    def _traceback(self, arg0, e):
        print(f"{arg0}{e}")
        import traceback
        traceback.print_exc()

    def _create_3d_axes(self, highlight_id, coords_3d):
        import matplotlib.pyplot as plt
        from mpl_toolkits.mplot3d import Axes3D  # Import 3D axes explicitly

        # Create figure and 3D axes with proper type
        fig = plt.figure(figsize=(10, 8))
        ax = fig.add_subplot(111, projection="3d")  # This returns Axes3D object
        ...
        for name, coords in coords_3d.items():
            # Collect scatter parameters in a dict to avoid parameter conflict issues
            scatter_kwargs = {
                "s": 100 if name != highlight_id else 200,  # Size
                "color": colors.get(name, "gray"),          # Color
                "marker": "o" if name != highlight_id else "*", # Marker
                "label": name                                # Label
            }
            
            # Call scatter with unpacked kwargs to avoid parameter conflicts
            ax.scatter(
                float(coords[0]),
                float(coords[1]),
                float(coords[2]),
        ...
        # Add labels and legend - explicitly use Axes3D methods
        ax.set_xlabel("Dimension 1")
        ax.set_ylabel("Dimension 2")
        # For 3D axes, use set_zlabel - cast to Axes3D to help type checker
        ax3d = cast(Axes3D, ax)
...
class APIAnalysisTool(BaseTool):
    ...
    def _run(self, api_spec: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            total_params = sum(
                len(endpoint.get("parameters", [])) for endpoint in endpoints
            )
            ...
            for endpoint in endpoints:
                for param in endpoint.get("parameters", []):
                    ...
                    if param_name:
                        ...
                        param_usage[param_name]["types"].add(
                            param.get("type", "unknown")
                        )
                        param_usage[param_name]["endpoints"].append(
            ...
            consistent_params = [
                p
                for p, data in param_usage.items()
                if len(data["types"]) == 1 and data["count"] > 1
            ]
            inconsistent_params = [
                p for p, data in param_usage.items() if len(data["types"]) > 1
            ]
            ...
            avg_coupling = (
                sum(coupling_degree.values()) / len(coupling_degree)
                if coupling_degree
                else 0
            )
            ...
            param_balance_score = 100 * (
                1.0 / (1.0 + abs(avg_params - 3.0))
            )  # Ideal: ~3 params per endpoint
            consistency_score = 100 * (
                len(consistent_params) / total_params if total_params > 0 else 0
            )
            coupling_score = 100 * (1.0 - avg_coupling)  # Lower coupling is better

            # Overall quality score (weighted average)
            quality_score = (
                0.3 * param_balance_score
            ...
            if inconsistent_params:
                recommendations.append(
                    ...
                    + (
                        f" and {len(inconsistent_params) - 3} more"
                        if len(inconsistent_params) > 3
                        else ""
                    )
                )
